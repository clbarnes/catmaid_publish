...
Shared utilities are in [`src/catmaid_publish/`](./src/catmaid_publish).
Scripts using those utilities are in [`scripts/`](./scripts).

### Data

Data files should be kept to the [`data/`](./data) directory,
or the path can be set using the `CATMAID_PUBLISH_DATA` environment variable
...
The path to this directory can be found in the python variable `catmaid_publish.constants.CREDENTIALS_DIR`.
...
You can find your package version at `catmaid_publish.__version__`.
